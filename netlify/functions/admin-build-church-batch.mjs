...
export default async (request) => {
  ...
  try {
    ...
    const [orders, batches] = await Promise.all([listStoreJSON('izhe-orders'), listStoreJSON('izhe-production-batches')]); const target = selectEditableChurchBatch(batches, campaignId); const selection = assembleChurchPickupItems({ campaign, orders, batches, targetBatch: target });
    if (!target && !selection.items.length) return json({ batch: null, created: false, updated: false, ordersIncluded: 0, unitsIncluded: 0, ordersExcluded: selection.excluded.length, exclusions: selection.excluded, message: 'No eligible unallocated paid church-pickup items are available.' });
    const store = getStore('izhe-production-batches'); const existingEntry = target ? await store.getWithMetadata(target.id, { type: 'json', consistency: 'strong' }) : null;
    ...
    const result = existingEntry ? await store.setJSON(batch.id, batch, { onlyIfMatch: existingEntry.etag }) : await store.setJSON(batch.id, batch, { onlyIfNew: true }); if (!result.modified) return json({ error: 'The church-pickup batch changed in another session. Reload and retry.' }, 409);
    await syncBatchSources(existingEntry?.data?.items || [], selection.items, batch); const exclusionOrders = new Set(selection.excluded.map((item) => item.sessionId).filter(Boolean));
    return json({ batch, created: !existingEntry, updated: Boolean(existingEntry), ordersIncluded: selection.ordersIncluded, unitsIncluded: selection.unitsIncluded, ordersExcluded: exclusionOrders.size, exclusions: selection.excluded }, existingEntry ? 200 : 201);
  } catch (error) { console.error('admin-build-church-batch', error); return json({ error: error.message || 'Church-pickup batch could not be assembled.' }, 400); }
};
