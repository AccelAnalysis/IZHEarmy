...
function orderRows(records) {
  return [
    ...
    ...records.map((order) => [
      ...
      order.currency,
      order.fulfillment?.mode || 'individual_shipping',
...
const EXPORTS = Object.freeze({
  orders: {
    store: 'izhe-orders',
    rows: orderRows,
    permission: 'operations.orders.export',
    statusResolver: (record) => record.status
  },
  redemptions: {
    store: 'izhe-redemptions',
    rows: redemptionRows,
    permission: 'operations.give_one.export',
    statusResolver: (record) => record.status
  },
  codes: {
    store: 'izhe-give-codes',
    rows: codeRows,
    permission: 'operations.give_one.export',
    statusResolver: effectiveCodeStatus
  },
  batches: {
    store: 'izhe-production-batches',
    rows: batchRows,
...
export default adminEndpoint({
  ...
  if (!hasPermission(context.permissions, config.permission)) {
    throw Object.assign(new Error('You do not have permission to export this operational dataset.'), { statusCode: 403 });
  }
  ...
  if (from && to && to - from > 92 * 24 * 60 * 60 * 1000) {
    throw Object.assign(new Error('Operational exports are limited to a 92-day date range.'), { statusCode: 400 });
  }
  ...
  let filtered = filterRecords(records, {
    q: text(body.search, 200),
    status: text(body.status, 80),
  ...
  if (filtered.length > maxRows) {
    throw Object.assign(new Error(`The export contains ${filtered.length} rows, exceeding the ${maxRows}-row limit. Narrow the filters.`), { statusCode: 400 });
  }

  const filename = `izhe-${type}-${new Date().toISOString().slice(0, 10)}.csv`;
  return {
    response: new Response(csv(config.rows(filtered)), {
      status: 200,
      headers: {
    ...
    audit: {
      ...
      afterSummary: {
        ...
        bounded: true,
        containsPII: ['orders', 'redemptions', 'codes'].includes(type)
      }
    }
  };
});
